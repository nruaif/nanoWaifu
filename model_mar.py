...
def get_2d_sincos_pos_embed(embed_dim, h, w, device):
    ...
    omega = torch.exp(-math.log(10000.0) * torch.arange(0, dim_half, dtype=torch.float32, device=device) / dim_half)
...
class TimestepEmbedder(nn.Module):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        half = dim // 2
        freqs = torch.exp(-math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32).to(device=t.device) / half)
...
class SizeEmbedder(nn.Module):
    ...
    def forward(self, h, w, bsz, device):
        h_freq = TimestepEmbedder.timestep_embedding(torch.tensor([h], dtype=torch.float32, device=device), self.frequency_embedding_size)
        w_freq = TimestepEmbedder.timestep_embedding(torch.tensor([w], dtype=torch.float32, device=device), self.frequency_embedding_size)
        hw_freq = torch.cat([h_freq, w_freq], dim=-1).expand(bsz, -1)
        return self.mlp(hw_freq)
...
class FMLoss(nn.Module):
    ...
    def forward(self, target, z, h_feat, w_feat, mask=None):
        ...
        t_base = torch.rand((B,), device=target.device)
    ...
    def sample(self, z, h_feat, w_feat, temperature=1.0, cfg=1.0):
        ...
        if cfg != 1.0:
            B = B // 2
            noise = torch.randn(B, seq_len, self.in_channels, device=z.device) * temperature
        ...
        ts = torch.linspace(1.0, 0.0, steps + 1, device=z.device)
...
class MARModel(nn.Module):
    def __init__(self, in_channels=128, dim=768, depth=12, num_heads=12, num_classes=12476, latent_size=16, use_checkpoint=False, 
                 mask_ratio_min=0.7, label_drop_prob=0.1, buffer_size=64, diffloss_d=3, diffloss_w=1024, num_sampling_steps=100, 
        ...
        self.encoder_blocks = nn.ModuleList([
            Block(dim, num_heads, 4.0, qkv_bias=True, norm_layer=nn.LayerNorm) for _ in range(depth)])
    ...
    def forward_mae_encoder(self, x, mask, class_embedding, h_, w_):
        ...
        pos_embed = get_2d_sincos_pos_embed(embed_dim, h_, w_, x.device)
        encoder_pos_embed = torch.cat([self.buffer_pos_embed_encoder, pos_embed], dim=1)
    ...
    def forward_mae_decoder(self, x, mask, h_, w_):
        ...
        pos_embed = get_2d_sincos_pos_embed(x.shape[-1], h_, w_, x.device)
        decoder_pos_embed = torch.cat([self.buffer_pos_embed_decoder, pos_embed], dim=1)
    ...
    def sample_tokens_custom(self, bsz, y_indices, y_offsets, num_classes, h_, w_, num_iter=64, cfg=1.0, cfg_schedule="linear", temperature=1.0):
        seq_len = h_ * w_
        ...
        for step in range(num_iter):
            ...
            if cfg != 1.0:
                ...
                uncond_indices = torch.full((bsz,), num_classes, dtype=torch.long, device=tokens.device)
                uncond_offsets = torch.arange(bsz, dtype=torch.long, device=tokens.device)
...
def sample_mar(model, tag_processor, latent_size, batch_size, prompts, device,
                steps=64, cfg_scale=1.4, noise=None):
    model.eval()
...
if __name__ == '__main__':
    # Test script for MARModel
    print("Testing MARModel instantiation and forward pass...")
    
    # Create dummy tag processor to avoid needing a real tags file
    class DummyTagProcessor:
        def __init__(self, num_classes):
            self.num_classes = num_classes
        def process_prompts(self, prompts, device, dropout_prob=0.0):
            bsz = len(prompts)
            # Create dummy indices and offsets
    ...
    # We use smaller dimensions for a quick test
    in_channels = 32
    num_classes = 1000
    model = MARModel(
    ...
    # Let's say our latent size is (16, 16) and each patch is 1x1.
    # The patchify method groups spatial patches. 
    # For a 16x16 grid with in_channels=32, the tensor is [B, 32, 16, 16]
    ...
    # 3. Test generation pass (sampling mode)
    ...
    print(f"Expected shape: [Batch, Channels, Height, Width] -> [{bsz}, {in_channels}, 16, 16]")
